...
# `cuda-bindings` (bindgen over cuda.h + curand.h) and therefore for type-checking risc0-groth16-cuda
...
for p in cuda-driver-dev-$series cuda-cudart-dev-$series cuda-crt-$series cuda-cccl-$series libcurand-dev-$series; do
  f=$(pick "$p"); [ -n "$f" ] || { echo "cuda-headers: no package $p in $repo" >&2; exit 1; }
...
[ -f "$root/include/cuda.h" ] || [ -f "$root/targets/x86_64-linux/include/cuda.h" ] || { echo "cuda-headers: cuda.h not found under $root" >&2; exit 1; }
echo "CUDA_HOME=$root"
