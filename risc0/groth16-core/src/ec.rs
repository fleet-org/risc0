...
pub fn g1_generator() -> G1Affine {
    Affine::new(Fp::from_u64(1), Fp::from_u64(2))
}

/// The BN254 G2 generator (EIP-197 / arkworks `G2Affine::generator()`), from
/// canonical little-endian limbs; `tests::generators` proves both against
/// arkworks.
pub fn g2_generator() -> G2Affine {
    ...
    Affine::new(
        Fp2::new(Fp::from_canonical(X_C0), Fp::from_canonical(X_C1)),
        Fp2::new(Fp::from_canonical(Y_C0), Fp::from_canonical(Y_C1)),
    )
}
...
mod tests {
    ...
    fn generators() {
        use ark_ec::AffineRepr as _;
        assert_eq!(
            conv::g1_to_ark(&g1_generator()),
            ark_bn254::G1Affine::generator()
        );
        assert_eq!(
            conv::g2_to_ark(&g2_generator()),
            ark_bn254::G2Affine::generator()
        );
        assert!(g1_generator().is_on_curve(&g1_b()));
        assert!(g2_generator().is_on_curve(&g2_b()));
    }
}
