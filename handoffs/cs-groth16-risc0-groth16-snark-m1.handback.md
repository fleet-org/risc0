...
| C16        | the production circuit through the Metal shaders on the CPU (`metal-cpu`): verifies, arms killed, prove 424.5 s; `prepare_owned` in both arms (peak 20.4 → 12.8 GB); `Registry::without`; the CUDA prover's per-phase timing switch; `gpu-free.py` for shared devices         | PR #24                                                                    |
